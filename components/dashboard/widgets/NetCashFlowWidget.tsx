...
export function NetCashFlowWidget({ view, locale, totalsByCurrency, currency }: NetCashFlowWidgetProps) {
  const rows = Object.entries(totalsByCurrency).map(([code, totals]) => ({
    currency: code,
    amountMinor: totals.netMinor,
    count: totals.incomeCount + totals.expenseCount,
  }));
