...
export async function POST(request: Request) {
  ...
  if (objectIds.some((id) => !id)) return errorResponse("Invalid merchant ids", 400);
  const ids = objectIds.filter((id) => id !== null);

  if (parsed.data.action === "delete") {
    const result = await MerchantModel.deleteMany({ workspaceId: auth.workspace.id, _id: { $in: ids } });
    return NextResponse.json({ data: { deleted: result.deletedCount } });
  }

  const result = await MerchantModel.updateMany(
