...
export async function POST(request: Request) {
  ...
  if (objectIds.some((id) => !id)) return errorResponse("Invalid category ids", 400);

  const ids = objectIds.filter((id) => id !== null);

  if (parsed.data.action === "delete") {
    await TransactionModel.updateMany(
      { workspaceId: auth.workspace.id, categoryId: { $in: ids } },
      { $set: { categoryId: null } }
    );
    const result = await CategoryModel.deleteMany({ workspaceId: auth.workspace.id, _id: { $in: ids } });
