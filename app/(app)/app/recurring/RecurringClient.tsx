...
export function RecurringClient({ locale, defaultCurrency }: { locale: Locale; defaultCurrency: string }) {
  ...
  const handleEdit = (item: Recurring) => {
    ...
    setFormState({
      ...
      dayOfMonth: String(item.schedule.dayOfMonth),
    });
  };
  ...
  const handleGenerate = async () => {
    try {
      await postJSON<ApiItemResponse<{ created: string[] }>>(
        `/api/recurring/run?month=${month}`,
        {}
      );
      setToast(null);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <label className="form-control">
            <span className="label-text text-xs">{t(locale, "recurring_month")}</span>
            <input
          ...
          <button className="btn btn-outline" type="button" onClick={handleGenerate}>
            {t(locale, "recurring_generate")}
          </button>
        </div>
      </div>
      ...
      <form className="space-y-4 rounded-box border border-base-300 bg-base-100 p-4" onSubmit={handleSubmit}>
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>{t(locale, "recurring_day_of_month")}</th>
                <th />
              </tr>
            </thead>
            <tbody>
              {recurring.map((item) => (
                <tr key={item._id} className={item.isActive ? "" : "opacity-60"}>
                  <td>{item.name}</td>
                  <td>
                  ...
                  <td>{item.schedule.dayOfMonth}</td>
